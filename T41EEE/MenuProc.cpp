...
/***** 11-22-2025 CMA KN6OVL
  Purpose: Present the Time Setting options available and set the system and RTC clocks
...
void TSOptions()  // Time Set Options 11-22-2025 CMA KN6OVL
{
  const char *tsChoices[]{ "SET TIME", "Cancel" };  // CMA 11-22-2025 KN60VL
  
  // const char *tsChoices[]{"HOUR"."MINUTES","Cancel"};
  ...
  TSChoice = SubmenuSelect(tsChoices, 2, 0);

  // TSChoice = SubmenuSelect(tsChoices,3,0);

  switch (TSChoice) {
    case 0:  // set time

   hr = GetEncoderValue(0,23,0,1,(char *)"Hours");
   delay(100); //avoid Select button bounce
   minutes = GetEncoderValue(0,59,0,1,(char *)"Minutes");
   delay(100);
   days = GetEncoderValue(1,31,1,1,(char *)"Day");
   delay(100);
   months = GetEncoderValue(1,12,1,1,(char *)"Month");
   delay(100);
   year = GetEncoderValue(25,99,25,1,(char *)"Year");
   delay(100);
  
  setTime(hr,minutes,00,days,months,year); //set the System clock
  Teensy3Clock.set(now()); //set the RTC

     // setTime(23,55,00,22,11,2025);
...
const char *topMenus[] = { "Bearing", "CW Options", "RF Set", "VFO Select",
                           ...
                           "EQ Rec Set", "EQ Xmt Set", "Calibrate", "Time" };    /*11-22-2025 CMA KN6OVL ADDED "Time" menu item */
